...
public class ArangoDBDocumentOutput extends BaseStep implements StepInterface {
  private static Class<?> PKG = ArangoDBDocumentOutputMeta.class; // for i18n purposes, needed by Translator2!!

  private ArangoDBDocumentOutputMeta meta;
  private ArangoDBDocumentOutputData data;
   ...
   */
  public ArangoDBDocumentOutput(StepMeta stepMeta, StepDataInterface stepDataInterface, int copyNr, TransMeta transMeta,
      Trans trans) {
    super(stepMeta, stepDataInterface, copyNr, transMeta, trans);

    meta = (ArangoDBDocumentOutputMeta) getStepMeta().getStepMetaInterface();
    data = (ArangoDBDocumentOutputData) stepDataInterface;
  }
  ...
  public boolean processRow(StepMetaInterface smi, StepDataInterface sdi) throws KettleException {
    meta = (ArangoDBDocumentOutputMeta) smi;
    data = (ArangoDBDocumentOutputData) sdi;
    ...
    if (first) {
      ...
      */
      ...
      try {
        ...
        logError(BaseMessages.getString(PKG, "ArangoDBDocumentOutput.Log.CannotConnect"), e);
      }

      logRowlevel("Processing first ArangoDB insert row prep completed");

    } // end if for first row (initialisation based on row data)

    // 3. putRow
    try {
      writeJsonDocument(data, r, data.db.collection((String) r[data.collectionFieldId]), data.docKeyFieldId,
          (String)r[data.contentFieldId]);

    } catch (Exception ex) {
      logError(BaseMessages.getString(PKG, "ArangoDBDocumentOutput.Log.ExceptionPuttingRow"), ex);
    }
    ...
    if (checkFeedback(getLinesRead())) {
      if (log.isBasic()) {
        logBasic(BaseMessages.getString(PKG, "ArangoDBDocumentOutput.Log.LineNumber") + getLinesRead());
      }
    }
  ...
  public void dispose(StepMetaInterface smi, StepDataInterface sdi) {
    ...
    logRowlevel("dispose called on ArangoDBDocumentOutput step");

    super.dispose(smi, sdi);
  }

  private void writeJsonDocument(ArangoDBDocumentOutputData data,Object[] row, ArangoCollectionAsync collection,int keyFieldId,String content) throws Exception {
    logRowlevel("writeJsonDocument called for document");
    final CompletableFuture<DocumentCreateEntity<String>> f = collection.insertDocument(content); // TODO replace this with a batch write method
	  f.whenComplete((doc, ex) -> {
      Object[] outputRowData = RowDataUtil.createResizedCopy( row, data.outputRowMeta.size() );
      logRowlevel("Document Exception: " + ex);
      outputRowData[keyFieldId] = doc.getKey();
      logRowlevel("Document Key: " + doc.getKey());
      // putRow
      try {
